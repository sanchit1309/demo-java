...
public class OpinionPage extends BaseTest{
	...
	public void launchApp(@Optional("0") int storiesDetailsTocheck, @Optional("3") int noOfSwipes,
		...
		apiMenuOptions = ApiHelper.getValueFromAPI(AppFeeds.lhsFeed, "nm", "nm", "Opinion", "Item", "ss");
		Assert.assertTrue(navigateToOpinionTab());
	}

	
	@Test(description = "Verifies Opinion header", priority = 0)
	public void verifyOpinionHeader() {
		String headerText = headerPageMethods.getHeaderText();
		Assert.assertEquals(headerPageMethods.getHeaderText(), "Opinion", "Actual header text:" + headerText);
	}
	
	@Test(description = "Verifies news in top news section", priority = 1, dataProvider = "top tabs")
	public void verifyTopNewsOpinion(String tabName) {
		...
		softAssert.assertTrue(headerAdFlag, "Header ad missing from Opinion Section, tab : "+tabName);
		WaitUtil.sleep(2000);
		softAssert.assertTrue(adTechMethods.isAdDisplayed("Footer"), "Footer ad missing from Opinion Section, tab : "+tabName);
		...
		softAssert.assertTrue(colombiaAdFlag, "Colombia ad missing from Opinion Section, tab : "+tabName);
		softAssert.assertTrue(mrecAdFlag, "Mrec ad missing from Opinion Section, tab : "+tabName);
	...
	private boolean navigateToOpinionTab() {
		headerPageMethods.clickMenuIcon();
		return new MenuPageMethods(appDriver).clickL1L2MenuByLabel("Opinion", "Top News");
	}
	
	
	
	private void verifyHeadlineNDate(MobileElement news, String headline, String tabName) {
		softAssert.assertTrue(!headline.equals("null"), "Headline not found under Opinion,tab:" + tabName);
		if (storiesToCheck > 0) {
			...
			if (headlineFlag == null || !headlineFlag)
				new ScreenShots().seleniumNativeScreenshot(appDriver,"headlineMissingOpinion");
			WaitUtil.sleep(1000);
