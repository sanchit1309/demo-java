...
public class WealthPage extends BaseTest {
	...
	private String sectionToExlude = "P2P,Calculators,Definition";
	...
	public void launchApp(@Optional("2") int storiesDetailsTocheck, @Optional("3") int noOfSwipes,
		...
		apiMenuOptions = ApiHelper.getValueFromAPI(AppFeeds.lhsFeed, "nm", "nm", "Wealth", "Item", "ss");
		Assert.assertTrue(navigateToWealthTab());
	}

	@Test(description = "Verifies wealth header", priority = 0)
	public void verifyWealthHeader() {

		String headerText = headerPageMethods.getHeaderText();
		Assert.assertEquals(headerPageMethods.getHeaderText(), "Wealth", "Actual header text:" + headerText);
	}

	@Test(description = "Verifies news in top news section", priority = 1, dataProvider = "top tabs")
	public void verifyTopNewsWealth(String tabName) {
		...
		softAssert.assertTrue(headerAdFlag, "Header ad missing from Wealth section, tab : "+tabName);
		softAssert.assertTrue(adTechMethods.isAdDisplayed("Footer"), "Footer ad missing from Wealth Section, tab : "+tabName);
		...
		while (swipesCounter >= 0) {
			...
			System.out.println(isStoryFound + "<-isStoryFound," + flag + "<-flag");
		...
		softAssert.assertTrue(colombiaAdFlag, "Colombia ad missing from Wealth Section, tab : "+tabName);
		softAssert.assertTrue(mrecAdFlag, "Mrec ad missing from Wealth Section, tab : "+tabName);
		softAssert.assertAll();

	}

	@Test(description = "Verifies top horizontal tabs and check if stories are loading", priority = 2)
	public void topTabsWealth() {
		softAssert = new SoftAssert();
		...
		System.out.println("apiMenuOption list ::: "+apiMenuOptions);
		apiMenuOptions.forEach(text -> {
			...
			isStoryFound = mapNews.size() > 0;
			if(isStoryFound==false){
				iAppCommonMethods.scrollDown();
				mapNews = appListingPageMethods.getHeadlineList();
				isStoryFound = mapNews.size() > 0;
			}
	...
	public Object[] getTopTabs() {
		...
		System.out.println("restrictedtblist :: "+restrictedTabsList);
		return restrictedTabsList.toArray(new Object[restrictedTabsList.size()]);
	}

	private boolean navigateToWealthTab() {
		headerPageMethods.clickMenuIcon();
		return new MenuPageMethods(appDriver).clickL1L2MenuByLabel("Wealth", "Top News");
	}

	private void verifyHeadlineNDate(MobileElement news, String headline, String tabName) {
		softAssert.assertTrue(!headline.equals("null"), "Headline not found under Wealth, tab:" + tabName);
		if (storiesToCheck > 0) {
			...
			if (headlineFlag == null || !headlineFlag)
				new ScreenShots().seleniumNativeScreenshot(appDriver, "headlineMissingWealth");
			WaitUtil.sleep(1000);
