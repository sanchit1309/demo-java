...
public class PanachePage extends BaseTest{
	...
	public void launchApp(@Optional("1") int storiesDetailsTocheck, @Optional("3") int noOfSwipes,
		...
		apiMenuOptions = ApiHelper.getValueFromAPI(AppFeeds.lhsFeed, "nm", "nm", "Panache", "Item", "ss");
		Assert.assertTrue(navigateToPanacheTab());
	}

	
	@Test(description = "Verifies Panache header", priority = 0)
	public void verifyPanacheHeader() {
		String headerText = headerPageMethods.getHeaderText();
		Assert.assertEquals(headerPageMethods.getHeaderText(), "Panache", "Actual header text:" + headerText);
	}
	
	@Test(description = "Verifies news in top news section", priority = 1, dataProvider = "top tabs")
	public void verifyTopNewsPanache(String tabName) {
		...
		softAssert.assertTrue(headerAdFlag, "Header ad missing from Panache Section, tab : " + tabName);
		WaitUtil.sleep(2000);
		softAssert.assertTrue(adTechMethods.isAdDisplayed("Footer"),
				"Footer ad missing from Panache Section, tab : " + tabName);
		...
		softAssert.assertTrue(colombiaAdFlag, "Colombia ad missing from Panache Section, tab : " + tabName);
		softAssert.assertTrue(mrecAdFlag, "Mrec ad missing from Panache Section, tab : " + tabName);
	...
	private boolean navigateToPanacheTab() {
		headerPageMethods.clickMenuIcon();
		return new MenuPageMethods(appDriver).clickL1L2MenuByLabel("Panache", "Panache Top News");
	}
	
	
	
	private void verifyHeadlineNDate(MobileElement news, String headline, String tabName) {
		softAssert.assertTrue(!headline.equals("null"), "Headline not found under Panache,tab:" + tabName);
		if (storiesToCheck > 0) {
			...
			if (headlineFlag == null || !headlineFlag)
				new ScreenShots().seleniumNativeScreenshot(appDriver, "headlineMissingPanache");
			WaitUtil.sleep(1000);
