...
public class NriPage extends BaseTest{
	...
	public void launchApp(@Optional("2") int storiesDetailsTocheck, @Optional("3") int noOfSwipes,
		...
		apiMenuOptions = ApiHelper.getValueFromAPI(AppFeeds.lhsFeed, "nm", "nm", "NRI", "Item", "ss");
		Assert.assertTrue(navigateToNRITab());
	}

	
	@Test(description = "Verifies NRI header", priority = 0)
	public void verifyNRIHeader() {
		String headerText = headerPageMethods.getHeaderText();
		Assert.assertEquals(headerPageMethods.getHeaderText(), "NRI", "Actual header text:" + headerText);
	}
	
	@Test(description = "Verifies news in top news section", priority = 1, dataProvider = "top tabs")
	public void verifyTopNewsNRI(String tabName) {
		...
		softAssert.assertTrue(headerAdFlag, "Header ad missing from NRI Section, tab : " + tabName);
		WaitUtil.sleep(2000);
		softAssert.assertTrue(adTechMethods.isAdDisplayed("Footer"),
				"Footer ad missing from NRI Section, tab : " + tabName);
		...
		softAssert.assertTrue(colombiaAdFlag, "Colombia ad missing from NRI Section, tab : " + tabName);
		softAssert.assertTrue(mrecAdFlag, "Mrec ad missing from NRI Section, tab : " + tabName);
	...
	private boolean navigateToNRITab() {
		headerPageMethods.clickMenuIcon();
		return new MenuPageMethods(appDriver).clickL1L2MenuByLabel("NRI", "Top News");
	}
	
	
	
	private void verifyHeadlineNDate(MobileElement news, String headline, String tabName) {
		softAssert.assertTrue(!headline.equals("null"), "Headline not found under NRI,tab:" + tabName);
		if (storiesToCheck > 0) {
			...
			if (headlineFlag == null || !headlineFlag)
				new ScreenShots().seleniumNativeScreenshot(appDriver, "headlineMissingNRI");
			WaitUtil.sleep(1000);
