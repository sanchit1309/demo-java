...
public class IndustryPage extends BaseTest {
	...
	String sectionToExclude="miscellaneous";
	...
	public void launchApp(@Optional("2") int storiesDetailsTocheck, @Optional("3") int noOfSwipes,
		...
		apiMenuOptions = ApiHelper.getValueFromAPI(AppFeeds.lhsFeed, "nm", "nm", "Industry", "Item", "ss");
		Assert.assertTrue(navigateToIndustryTab());
	}

	@Test(description = "Verifies industry page header", priority = 0)
	public void verifyIndustryHeader() {
		String headerText = headerPageMethods.getHeaderText();
		Assert.assertEquals(headerPageMethods.getHeaderText(), "Industry", "Actual header text:" + headerText);
	}

	@Test(description = "Verifies news in top news section", priority = 1, dataProvider = "top tabs",enabled=true)
	public void verifyTopNewsIndustry(String tabName) {
		...
		softAssert.assertTrue(headerAdFlag, "Header ad missing from Industry Section, tab : "+tabName);
		WaitUtil.sleep(2000);
		softAssert.assertTrue(adTechMethods.isAdDisplayed("Footer"), "Footer ad missing from Industry Section, tab : "+tabName);
		...
		softAssert.assertTrue(colombiaAdFlag, "Colombia ad missing from Industry Section, tab : "+tabName);
		softAssert.assertTrue(mrecAdFlag, "Mrec ad missing from Industry Section, tab : "+tabName);
		softAssert.assertAll();

	}

	@Test(description = "Verifies top horizontal tabs and check if stories are loading", priority = 2,enabled=true)
	public void topTabsIndustry() {
		softAssert = new SoftAssert();
		...
		apiMenuOptions.forEach(text -> {
			...
			if(!sectionToExclude.contains(text))
	...
	private boolean navigateToIndustryTab() {
		headerPageMethods.clickMenuIcon();
		return new MenuPageMethods(appDriver).clickL1L2MenuByLabel("Industry", "Top News");
	}

	private void verifyHeadlineNDate(MobileElement news, String headline, String tabName) {
		softAssert.assertTrue(!headline.equals("null"), "Headline not found under Industry,tab:" + tabName);
		if (storiesToCheck > 0) {
			...
			if (headlineFlag == null || !headlineFlag)
				new ScreenShots().seleniumNativeScreenshot(appDriver,"headlineMissingIndustry");
			WaitUtil.sleep(1000);
