...
public class PoliticsPage extends BaseTest {
	...
	public void launchApp(@Optional("2") int storiesDetailsTocheck, @Optional("3") int noOfSwipes) throws IOException {
		...
		Assert.assertTrue(navigateToPoliticsTab());
	}

	@Test(description = "Verifies politics header", priority = 0)
	public void verifyPoliticsHeader() {
		String headerText = headerPageMethods.getHeaderText();
		Assert.assertEquals(headerPageMethods.getHeaderText(), "Politics", "Actual header text:" + headerText);
	}
	
	@Test(description = "Verifies news in top news section", priority = 1)
	public void verifyTopNewsPolitics() {
		...
		softAssert.assertTrue(headerAdFlag, "Header ad missing from Politics section");
		softAssert.assertTrue(adTechMethods.isAdDisplayed("Footer"), "Footer ad missing from Politics Section");
		...
		while (swipesCounter >= 0) {
			...
			softAssert.assertTrue(flag,
					"<br>-Zero stories found in top news, tab: Politics in swipe:" + (listPageSwipes - swipesCounter));
			swipesCounter = appListingPageMethods.swipeUpListPage(false, swipesCounter);
		}
		softAssert.assertTrue(colombiaAdFlag, "Colombia ad missing from Politics Section");
		softAssert.assertTrue(mrecAdFlag, "Mrec ad missing from Politics Section");
		softAssert.assertAll();
	}

	private boolean navigateToPoliticsTab() {
		headerPageMethods.clickMenuIcon();
		return new MenuPageMethods(appDriver).clickMenuByLabel("Politics");
	}

	private void verifyHeadlineNDate(MobileElement news, String headline) {
		softAssert.assertTrue(!headline.equals("null"), "Headline not found under Politics");
		if (storiesToCheck > 0) {
			...
			if(headlineFlag==null ||!headlineFlag)
				new ScreenShots().seleniumNativeScreenshot(appDriver,"headlineMissingPolitics");
			WaitUtil.sleep(1000);
