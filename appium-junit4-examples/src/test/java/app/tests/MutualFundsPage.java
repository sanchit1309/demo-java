...
public class MutualFundsPage extends BaseTest {
	...
	public void launchApp(@Optional("2") int storiesDetailsTocheck, @Optional("3") int noOfSwipes,
			@Optional("4") int noOfTabs) throws IOException {
		launchBrowser();
		...
		apiMenuOptions = ApiHelper.getValueFromAPI(AppFeeds.lhsFeed, "nm", "nm", "Mutual Funds", "Item", "ss");
		Assert.assertTrue(navigateToNewsTab());
	}

	@Test(description = "Verifies mutual funds header", priority = 0)
	public void verifyMutualFundHeader() {

		String headerText = headerPageMethods.getHeaderText();
		Assert.assertEquals(headerPageMethods.getHeaderText(), "Mutual Funds", "Actual header text:" + headerText);
	}

	@Test(description = "Verifies news in top news section", priority = 0, dataProvider = "top tabs")
	public void verifyTopNewMFs(String tabName) {
		...
		softAssert.assertTrue(headerAdFlag, "Header ad missing from MF section, tab : "+tabName);
		softAssert.assertTrue(adTechMethods.isAdDisplayed("Footer"), "Footer ad missing from MF Section, tab : "+tabName);
		...
		while (swipesCounter >= 0) {
			if(!colombiaAdFlag){
				if(BaseTest.platform.contains("ios"))
					iAppCommonMethods.swipeByScreenPercentage(0.55, 0.50);
		...
		softAssert.assertTrue(colombiaAdFlag, "Colombia ad missing from MF Section, tab : "+tabName);
		softAssert.assertTrue(mrecAdFlag, "Mrec ad missing from MF Section, tab : "+tabName);
		softAssert.assertAll();

	}

	@Test(description = "Verifies top horizontal navigation", priority = 1)
	public void topTabsMF() {
		softAssert = new SoftAssert();
	...
	private boolean navigateToNewsTab() {
		...
		return new MenuPageMethods(appDriver).clickL1L2MenuByLabel("Mutual Funds", "Top News");
	}
	...
	private void verifyHeadlineNDate(MobileElement news, String headline, String tabName) {
		...
		if (storiesToCheck > 0) {
			...
			if (headlineFlag == null || !headlineFlag)
				new ScreenShots().seleniumNativeScreenshot(appDriver, "headlineMissingMF");
			WaitUtil.sleep(1000);
