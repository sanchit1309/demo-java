...
import wap.pagemethods.Budget2021PageMethods;
...
public class Budget2021Page extends BaseTest {
	...
	public void beforeClass() throws IOException {
		wapUrl = BaseTest.baseUrl + "/budget";
	...
	public String[] subSections() {
		...
		String[] subSections = { "Budget Headlines", "Thirty Shots", "Know Your Macro", "Markets & Budget",
				"Expert Views", "Budget Bytes", "Taxes & You", "Common Man" };
		return subSections;
	...
	public void verifyMoreFromLinks() {
		...
		List<String> duplicateLinks = WebBaseMethods.checkIfListIsUnique(urlList);
		softAssert.assertTrue(size >= articleCount,
				"More from section links are not shown on few sections on the budget page " + articleCount
						+ " but found: " + size + ".");
		softAssert.assertTrue(errorLinks.size() == 0,
				"Few More from section links are not having the response code 200. List of such links are: "
						+ errorLinks);
		softAssert.assertTrue(duplicateLinks.size() == 0,
				"Duplicate links are there for more from section links. List of such links are: " + duplicateLinks);
		softAssert.assertAll();
	}
	...
	@Test(description = "This test verifies the budget In hindi links on the budget page")
	public void verifyBudgetInHindiLinks() {
		softAssert = new SoftAssert();
		WaitUtil.waitForLoad(driver);
		int articleCount = 5;
		List<String> urlList = budget2021PageMethods.getBudgetInHindiLinks();
		int size = urlList.size();
		List<String> errorLinks = WebBaseMethods.getAllErrorLinks(urlList);
		List<String> duplicateLinks = WebBaseMethods.checkIfListIsUnique(urlList);
		softAssert.assertTrue(size >= articleCount,
				"The number of links shown under the budget In hindi section should be greater than equal to "
						+ articleCount + " but found: " + size + ".");
		softAssert.assertTrue(errorLinks.size() == 0,
				"Few links under the budget In hindi are not having the response code 200. List of such links are: "
						+ errorLinks);
		softAssert.assertTrue(duplicateLinks.size() == 0,
				"Duplicate links are shown under the budget in hindi. List of such links are: " + duplicateLinks);
		softAssert.assertAll();
	}

	@Test(description = "This test verifies the budget archive links on the budget page")
	public void verifyBudgetArchiveLinks() {
		...
		softAssert.assertTrue(size >= articleCount,
				"The number of links shown under the budget Archive section should be greater than equal to "
						+ articleCount + " but found: " + size + ".");
		softAssert.assertTrue(errorLinks.size() == 0,
				"Few links under the budget archive are not having the response code 200. List of such links are: "
						+ errorLinks);
		softAssert.assertTrue(duplicateLinks.size() == 0,
				"Duplicate links are shown under the budget archive. List of such links are: " + duplicateLinks);
		softAssert.assertAll();
	}
